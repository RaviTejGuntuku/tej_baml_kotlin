/// Macros for testing Kotlin type serialization.
///
/// Test Kotlin type serialization with auto-generated test names.
///
/// # Examples
///
/// ```ignore
...
macro_rules! test_kt_type {
    ...
    (
        ...
        use $crate::ir_to_kotlin::classes::{ir_class_to_kotlin, ir_class_to_kotlin_stream};
        ...
        if parts.len() == 2 {
            ...
            let class_kt = ir_class_to_kotlin(class, &pkg);
            let field = class_kt
                .fields
                .iter()
            ...
            let class_kt_stream = ir_class_to_kotlin_stream(class, &pkg);
            ...
            use $crate::ir_to_kotlin::type_aliases::{
                ir_type_alias_to_kotlin, ir_type_alias_to_kotlin_stream,
            };
            ...
            // Non-streaming
            pkg.set("baml_client.types");
            let alias_kt = ir_type_alias_to_kotlin(type_alias, &pkg, None);
            assert_eq!(
                alias_kt.type_.serialize_type(&pkg),
                $expected_non_streaming,
            ...
            // Streaming
            pkg.set("baml_client.stream_types");
            let alias_kt_stream = ir_type_alias_to_kotlin_stream(type_alias, &pkg, None);
            assert_eq!(
                alias_kt_stream.type_.serialize_type(&pkg),
                $expected_streaming,
    ...
    (
        ...
        use internal_baml_core::ir::{repr::make_test_ir, IRHelper};
        use $crate::ir_to_kotlin::enums::ir_enum_to_kotlin;
        use $crate::package::CurrentRenderPackage;
        ...
        assert_eq!(enum_kt.name, $enum_name);

        let expected_values: Vec<&str> = vec![$( $value ),*];
        let actual_values: Vec<&str> = enum_kt.values.iter().map(|(v, _)| v.as_str()).collect();
        assert_eq!(
            actual_values,
            expected_values,
...
#[macro_export]
macro_rules! test_kt_types {
    ( $( ( $baml:expr, $path:expr, $non_streaming:expr, $streaming:expr ) ),* $(,)? ) => {
        $(
            $crate::test_kt_type!($baml, $path, $non_streaming, $streaming);
        )*
    };
}
